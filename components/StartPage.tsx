...
const StartPage: React.FC<StartPageProps> = ({ onStart }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (info.name && info.class && info.studentId) {
      onStart(info);
    }
  };
  ...
  return (
    <div className="max-w-md mx-auto p-8 bg-white rounded-2xl shadow-xl text-center">
      <div className="mb-8">
        ...
        <p className="text-slate-500 mt-2 font-medium">أهلاً بكم في امتحان التمرن. لديكم 30 دقيقة.</p>
      </div>

      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label className="block text-right text-sm font-semibold text-slate-700 mb-1">الاسم الكامل</label>
          <input
            required
            type="text"
        ...
        <div>
          <label className="block text-right text-sm font-semibold text-slate-700 mb-1">الصف</label>
          <input
            required
            type="text"
            placeholder="مثلاً: الثامن 1"
            className="w-full p-3 rounded-lg border border-slate-300 focus:ring-2 focus:ring-indigo-500 outline-none"
            value={info.class}
        ...
        <div>
          <label className="block text-right text-sm font-semibold text-slate-700 mb-1">رقم الهوية</label>
          <input
            required
            type="text"
            className="w-full p-3 rounded-lg border border-slate-300 focus:ring-2 focus:ring-indigo-500 outline-none"
